...
Setup:
  1. Get a FREE API key (no credit card needed): https://aistudio.google.com/apikey
  2. Create a .env file with: GOOGLE_API_KEY=your-key-here

Using Google's Gemini (gemini-2.5-flash) instead of GPT-5.5 because it has
a genuinely free tier for the Gemini API. GPT-5.5 is pay-per-token only.
==========================================================
"""
import warnings
...
# gemini-2.5-flash is on Google's FREE tier (as of writing) — no billing required.
model = init_chat_model("google_genai:gemini-3.6-flash")
# model = init_chat_model("openai:gpt-5.5")            # paid, needs OpenAI billing
# model = init_chat_model("anthropic:claude-3-5-sonnet-latest")  # paid, needs Anthropic billing
